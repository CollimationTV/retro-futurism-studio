...
const Index = () => {
  ...
  // Auto-navigate to Training when connection is ready and headsets are connected
  useEffect(() => {
    if (connectionStatus === 'ready' && connectedHeadsets.length > 0) {
      setTimeout(() => {
        navigate("/training", { state: { connectedHeadsets, mentalCommand, motionEvent } });
      }, 500);
    }
  ...
  return (
    <div className="min-h-screen relative">
        ...
        <section className="py-12 px-6">
          <div className="container mx-auto max-w-2xl text-center">
            <div className="p-8 rounded-lg border border-border/50 bg-card/30 backdrop-blur-sm">
              ...
              <h3 className="text-2xl font-bold mb-2">Launching Training...</h3>
              <p className="text-muted-foreground">Navigating to Mental Command Training</p>
            </div>
          </div>
        </section>
      )}
      ...
      <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 flex gap-2 bg-background/90 backdrop-blur-sm border border-border rounded-lg p-2">
        <button
          onClick={() => navigate("/training", { state: { connectedHeadsets, mentalCommand, motionEvent } })}
